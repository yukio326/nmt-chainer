...
class Encoder(Chain):
    ...
    def __init__(self, source_vocabulary_size, embed_size, hidden_size, source_vocabulary, source_word2vec, use_dropout, dropout_rate):
    ...
    def __call__(self, sentence):
        ...
        for word in sentence:
            embed_states.append(functions.dropout(functions.tanh(self.word2embed(word)), ratio = self.dropout_rate, train = self.use_dropout))
        for embed in embed_states[::-1]:
            hidden_backward_states.insert(0, functions.dropout(functions.tanh(self.embed2hidden_backward(embed)), ratio = self.dropout_rate, train = False))
        for embed, hidden_backward in zip(embed_states, hidden_backward_states):
            plus = functions.dropout(functions.tanh(self.embed2hidden_forward(embed)), ratio = self.dropout_rate, train = False) + hidden_backward
    ...
    def __call__(self, encoder_hidden_states, sentence):
        if sentence is not None:
            ...
            for i, word in enumerate(sentence):
                if i == 0:
                    hidden = functions.dropout(functions.tanh(self.encoder2decoder_init(encoder_hidden_states[0])), ratio = self.dropout_rate, train = False)
                    encoder_hidden_states = functions.dstack(encoder_hidden_states)
                    self.copy_states()
                else:
                    previous_embed = functions.dropout(functions.tanh(self.word2embed(sentence[i - 1])), ratio = self.dropout_rate, train = self.use_dropout)
                    hidden = functions.dropout(functions.tanh(self.embed_hidden_tilde2hidden(functions.concat((previous_embed, hidden_tilde)))), ratio = self.dropout_rate, train = False)
                ...
                hidden_tilde = functions.dropout(functions.tanh(self.attention_hidden2hidden_tilde(functions.concat((attention, hidden)))), ratio = self.dropout_rate, train = self.use_dropout)
            ...
            for i in range(self.generation_limit):
                ...
                if i == 0:
                    ...
                    for predict in numpy.argsort(cuda.to_cpu(prob.data)[0])[-1:-self.beam_size-1:-1]:
                        ...
                        if predict == 1:
                            beam[i].append((functions.log(prob[0][predict]), None, [predict_variable])) 
                        else:
                            beam[i].append((functions.log(prob[0][predict]), (cell, hidden, hidden_tilde), [predict_variable]))
                else:
                    new_beam = list()
                    for logprob, states, sentence in beam[i - 1]:
                        if states is not None:
                            ...
                            for predict in numpy.argsort(cuda.to_cpu(prob.data)[0])[-1:-self.beam_size-1:-1]:
                                predict_variable = Variable(self.library.array([predict], dtype = self.library.int32))
                                if predict == 1:
                                    new_beam.append((logprob + functions.log(prob[0][predict]), None, sentence + [predict_variable])) 
                                else:
                                    new_beam.append((logprob + functions.log(prob[0][predict]), (cell, hidden, hidden_tilde), sentence + [predict_variable]))
                        else:
                            new_beam.append((logprob, None, sentence))
                    for _, (logprob, states, sentence) in zip(range(self.beam_size), sorted(new_beam, key = lambda x: x[0].data / len(x[2]), reverse = True)):
                        beam[i].append((logprob, states, sentence))
            for _, _, sentence in sorted(beam[-1], key = lambda x: x[0].data / len(x[2]), reverse = True):
                predicts = list()
                for word in sentence:
...
class AttentionalNMT(Chain):
    def __init__(self, source_vocabulary_size, target_vocabulary_size, embed_size, hidden_size, source_vocabulary, target_vocabulary, source_word2vec, target_word2vec, use_dropout, dropout_rate, generation_limit, use_beamsearch, beam_size, library):
        super(AttentionalNMT, self).__init__(
            encoder = Encoder(source_vocabulary_size, embed_size, hidden_size, source_vocabulary, source_word2vec, use_dropout, dropout_rate),
...
def train(config):
    ...
    trace("Making Model ...")
    nmt = AttentionalNMT(config.source_vocabulary_size, config.target_vocabulary_size, config.embed_size, config.hidden_size, source_vocabulary, target_vocabulary, source_word2vec, target_word2vec, config.use_dropout, config.dropout_rate, None, None, None, config.library)
    if config.use_gpu:
...
def test(config):
    ...
    with open(config.predict_file, 'w') as ft:
        for batch_source in mono_batch(config.source_file, source_vocabulary, 1, config.library):
            ...
            _, batch_predict = nmt(batch_source, None)
